import React from 'react';
import CocaColaIcon from "svg-react-loader?name=CocaColaIcon!./../../assets/img/clients/clientCocaCola.svg";
import ACHIcon from "svg-react-loader?name=ACHIcon!./../../assets/img/clients/clientACH.svg";
import CottIcon from "svg-react-loader?name=CottIcon!./../../assets/img/clients/clientCott.svg";
import GrupoModeloIcon from "svg-react-loader?name=GrupoModeloIcon!./../../assets/img/clients/clientGrupoModelo.svg";

import PepsicoIcon from "svg-react-loader?name=PepsicoIcon!./../../assets/img/clients/clientPepsico.svg";
import LattaIcon from "svg-react-loader?name=LattaIcon!./../../assets/img/clients/clientLatta.svg";
import JanssenIcon from "svg-react-loader?name=JanssenIcon!./../../assets/img/clients/clientJanssen.svg";
import DiageoIcon from "svg-react-loader?name=DiageoIcon!./../../assets/img/clients/clientDiageo.svg";

import ChanelIcon from "svg-react-loader?name=ChanelIcon!./../../assets/img/clients/clientChanel.svg";
import HPIcon from "svg-react-loader?name=HPIcon!./../../assets/img/clients/clientHP.svg";
import ArlaIcon from "svg-react-loader?name=ArlaIcon!./../../assets/img/clients/clientArla.svg";
import FritoLayIcon from "svg-react-loader?name=FritoLayIcon!./../../assets/img/clients/clientFritoLay.svg";

import BossIcon from "svg-react-loader?name=BossIcon!./../../assets/img/clients/clientBoss.svg";
import BiersdorfIcon from "svg-react-loader?name=BiersdorfIcon!./../../assets/img/clients/clientBiersdorf.svg";
import BelgacomIcon from "svg-react-loader?name=BelgacomIcon!./../../assets/img/clients/clientBelgacom.svg";
import KelloggsIcon from "svg-react-loader?name=KelloggsIcon!./../../assets/img/clients/clientKelloggs.svg";

import PradaIcon from "svg-react-loader?name=PradaIcon!./../../assets/img/clients/clientPrada.svg";
import BonaguaIcon from "svg-react-loader?name=BonaguaIcon!./../../assets/img/clients/clientBonagua.svg";
import CaptainMorganIcon from "svg-react-loader?name=CaptainMorganIcon!./../../assets/img/clients/clientCaptainMorgan.svg";
import SigmaIcon from "svg-react-loader?name=SigmaIcon!./../../assets/img/clients/clientSigma.svg";

import KraftIcon from "svg-react-loader?name=KraftIcon!./../../assets/img/clients/clientKraft.svg";
import CarteDorIcon from "svg-react-loader?name=CarteDorIcon!./../../assets/img/clients/clientCarteDor.svg";
import PedroDomeqIcon from "svg-react-loader?name=PedroDomeqIcon!./../../assets/img/clients/clientPedroDomeq.svg";
import MoneyAmigoIcon from "svg-react-loader?name=MoneyAmigoIcon!./../../assets/img/clients/clientMoneyAmigo.svg";

import SanofiIcon from "svg-react-loader?name=SanofiIcon!./../../assets/img/clients/clientSanofi.svg";
import CadburyIcon from "svg-react-loader?name=CadburyIcon!./../../assets/img/clients/clientCadbury.svg";
import TequilaBlackIcon from "svg-react-loader?name=TequilaBlackIcon!./../../assets/img/clients/clientTequilaBlack.svg";
import MilkaIcon from "svg-react-loader?name=MilkaIcon!./../../assets/img/clients/clientMilka.svg";

import PfizerIcon from "svg-react-loader?name=PfizerIcon!./../../assets/img/clients/clientPfizer.svg";
import KendrickIcon from "svg-react-loader?name=KendrickIcon!./../../assets/img/clients/clientKendrick.svg";
import SymriseIcon from "svg-react-loader?name=SymriseIcon!./../../assets/img/clients/clientSymrise.svg";

import NovartisIcon from "svg-react-loader?name=NovartisIcon!./../../assets/img/clients/clientNovartis.svg";
import TrygIcon from "svg-react-loader?name=TrygIcon!./../../assets/img/clients/clientTryg.svg";
import UnileverIcon from "svg-react-loader?name=UnileverIcon!./../../assets/img/clients/clientUnilever.svg";

import NikeIcon from "svg-react-loader?name=NikeIcon!./../../assets/img/clients/clientNike.svg";
import CheetosIcon from "svg-react-loader?name=CheetosIcon!./../../assets/img/clients/clientCheetos.svg";
import FemsaIcon from "svg-react-loader?name=FemsaIcon!./../../assets/img/clients/clientFemsa.svg";

import MoneyGramIcon from "svg-react-loader?name=MoneyGramIcon!./../../assets/img/clients/clientMoneyGram.svg";
import ChicolasticIcon from "svg-react-loader?name=ChicolasticIcon!./../../assets/img/clients/clientChicolastic.svg";
import CipralexIcon from "svg-react-loader?name=CipralexIcon!./../../assets/img/clients/clientCipralex.svg";

import CapitalOneIcon from "svg-react-loader?name=CapitalOneIcon!./../../assets/img/clients/clientCapitalOne.svg";
import NovoNordiskIcon from "svg-react-loader?name=NovoNordiskIcon!./../../assets/img/clients/clientNovoNordisk.svg";
import EricssonIcon from "svg-react-loader?name=EricssonIcon!./../../assets/img/clients/clientEricsson.svg";

import BaseFIcon from "svg-react-loader?name=BaseFIcon!./../../assets/img/clients/clientBaseF.svg";
import CitroFruitIcon from "svg-react-loader?name=CitroFruitIcon!./../../assets/img/clients/clientCitroFruit.svg";
import PoweradeIcon from "svg-react-loader?name=PoweradeIcon!./../../assets/img/clients/clientPowerade.svg";

import AmigoincIcon from "svg-react-loader?name=AmigoincIcon!./../../assets/img/clients/clientAmigoinc.svg";
import PuigIcon from "svg-react-loader?name=PuigIcon!./../../assets/img/clients/clientPuig.svg";
import TequilaRealIcon from "svg-react-loader?name=TequilaRealIcon!./../../assets/img/clients/clientTequilaReal.svg";

import BMWIcon from "svg-react-loader?name=BMWIcon!./../../assets/img/clients/clientBMW.svg";
import SabritasIcon from "svg-react-loader?name=SabritasIcon!./../../assets/img/clients/clientSabritas.svg";
import RufflesIcon from "svg-react-loader?name=RufflesIcon!./../../assets/img/clients/clientRuffles.svg";

import CampbellsIcon from "svg-react-loader?name=CampbellsIcon!./../../assets/img/clients/clientCampbells.svg";
import TequilaPinqIcon from "svg-react-loader?name=TequilaPinqIcon!./../../assets/img/clients/clientTequilaPinq.svg";
import BimboIcon from "svg-react-loader?name=BimboIcon!./../../assets/img/clients/clientBimbo.svg";

const clientsData = {
  clientBoxes: [
    {
      id: 1,
      clients: [
        {
          icon: <CocaColaIcon />,
          quote:
            "Innovation of multiple products, brands, strategies, packaging’s and positioning’s across multiple categories, countries and continents"
        },
        {
          icon: <ACHIcon />,
          quote:
            "Brand and product innovation across multiple brands and categories"
        },
        {
          icon: <CottIcon />,
          quote:
            "A-to-Z brand development of water, spirit, tea and mix products. IN charge of all brand and marketing of brand portfolio."
        },
        {
          icon: <GrupoModeloIcon />,
          quote:
            "Innovation across multiple categories; product, brand and packaging"
        }
      ]
    },
    {
      id: 2,
      clients: [
        {
          icon: <PepsicoIcon />,
          quote:
            "Global A-to-Z innovation projects and brand positioning of all brands in specific category"
        },
        {
          icon: <LattaIcon />,
          quote:
            "By Unilever. Transformational innovation to combat category copycats. Results: 17% revenue growth and 89% brand loyalty."
        },
        {
          icon: <JanssenIcon />,
          quote:
            "Complete sales strategy for multiple therapeutical areas. Physician and consumer marketing. Result: Exceeding growth by 400%."
        },
        {
          icon:<DiageoIcon/>,
          quote: "Innovation across multiple categories; product, brand and packaging"
        }
      ]
    },
    {
      id: 3,
      clients: [
        {
          icon: <ChanelIcon />,
          quote:
            "Innovation of new exclusive packaging & brand & communication design"
        },
        {
          icon: <HPIcon />,
          quote:
            "Launch strategy and execution of ‘back-to-school’ laptops. All major channels including Costco, Sam’s Club and stand-alone stores"
        },
        {
          icon: <ArlaIcon />,
          quote:
            "Create and validate product innovations for world’s largest dairy market: China"
        },
        {
          icon: <FritoLayIcon />,
          quote:
            "Global A-to-Z innovation project for new product category across 4 continents; creation of leading category"
        }
      ]
    },
    {
      id: 4,
      clients: [
        {
          icon: <BossIcon />,
          quote: "Brand and packaging innovation for men’s line"
        },
        {
          icon: <BiersdorfIcon />,
          quote:
            "Brand, product & packaging Innovation for NIVEA personal care and make-up brand NIVEA Beauté etc."
        },
        {
          icon: <BelgacomIcon />,
          quote:
            "Transformational innovation: Brand, positioning and consumer marketing"
        },
        {
          icon: <KelloggsIcon />,
          quote:
            "New category Innovation as well as re-positioning of hero breakfast products in leading markets"
        }
      ]
    },
    {
      id: 5,
      clients: [
        {
          icon: <PradaIcon />,
          quote:
            "Conceptual and Packaging Innovation claiming unprecedented market share and cementing positioning as leading luxury brand"
        },
        {
          icon: <BonaguaIcon />,
          quote:
            "Creation of European strategy for most profitable category: water"
        },
        {
          icon: <CaptainMorganIcon />,
          quote: "Re-invention brand and new product innovation"
        },
        {
          icon: <SigmaIcon/>,
          quote: "Innovation across multiple categories; product, brand and packaging"
        }
      ]
    },
    {
      id: 6,
      clients: [
        {
          icon: <KraftIcon />,
          quote: "Innovation across multiple categories; product, brand and packaging"
        },
        {
          icon: <CarteDorIcon />,
          quote:
            "Product innovation and brand re-positioning towards horeca (restaurant industry)"
        },
        {
          icon: <PedroDomeqIcon />,
          quote: "Re-invention of existing and new spirit brands across categories"
        },
        {
          icon: <MoneyAmigoIcon/>,
          quote: "A-to-Z innovation of new cutting edge life solution; merging finance and health: Eco-system, brand, digital / IT and communication"
        }
      ]
    },
    {
      id: 7,
      clients: [
        {
          icon: <SanofiIcon />,
          quote: "Brand and positioning strategy of multiple therapeutical areas"
        },
        {
          icon: <CadburyIcon />,
          quote:
            "Transformational product, brand and packaging innovation across multiple categories."
        },
        {
          icon: <TequilaBlackIcon />,
          quote: "Innovation of new brand & product category; world’s first black Tequila"
        },
        {
          icon: <MilkaIcon/>, 
          quote: "New low-calorie category innovation + new brand and product innovation for kids category"
        }
      ]
    },
    {
      id: 8,
      clients: [
        {
          icon: <PfizerIcon />,
          quote: "Brand and communication strategy for new category product"
        },
        {
          icon: <KendrickIcon />,
          quote:
            "Innovation and creation of national generic hero brand from ground up. Acquired by Sanofi and continued as leading brand"
        },
        {
          icon: <SymriseIcon />,
          quote: "Global product and application innovation towards world’s largest flavor company"
        },
        {
          icon: <NovartisIcon />,
          quote: "Brand and positioning strategy of multiple therapeutical areas"
        }
      ]
    },
    {
      id: 9,
      clients: [
        {
          icon: <NovartisIcon />,
          quote: "Brand and positioning strategy of multiple therapeutical areas"
        },
        {
          icon: <TrygIcon />,
          quote:"Repositioning Scandinavia’s largest insurance company via experience innovation"
        },
        {
          icon: <UnileverIcon />,
          quote: "Innovation of multiple products, brands, strategies, packaging’s and positioning’s across multiple categories, countries and continents"
        },
        {
          icon: <NikeIcon />,
          quote: "Insight and innovation exploration of running and female apparel. Indentfied winning concepts for future trens and business segments"
        }
      ]
    },
    {
      id: 10,
      clients: [
        {
          icon: <NikeIcon />,
          quote: "Insight and innovation exploration of running and female apparel. Indentfied winning concepts for future trens and business segments"
        },
        {
          icon: <CheetosIcon />,
          quote:"Re-positioning strategy of hero brand in main market"
        },
        {
          icon: <FemsaIcon />,
          quote: "Business & Positioning innovation of World’s largest Coca-Cola Boteler Coca-Cola FEMSA (NYSE)"
        }, 
        {
          icon: <MoneyGramIcon />,
          quote: "Global A-to-Z re-invention and innovation of brand and new products / services. Communication strategy and campaign B2C and B2B"
        }
      ]
    },
    {
      id: 11,
      clients: [
        {
          icon: <MoneyGramIcon />,
          quote: "Global A-to-Z re-invention and innovation of brand and new products / services. Communication strategy and campaign B2C and B2B"
        },
        {
          icon: <ChicolasticIcon />,
          quote:"Re-imagination and rebranding of entire product line, brand"
        },
        {
          icon: <CipralexIcon />,
          quote: "Re-positioning, sales & marketing strategy of most important therapeutical product/brand."
        }, 
        {
          icon: <CapitalOneIcon />,
          quote: "Innovation of new product, brand and category including validation with core target"
        }
      ]
    },
    {
      id: 12,
      clients: [
        {
          icon: <CapitalOneIcon />,
          quote: "Innovation of new product, brand and category including validation with core target"
        },
        {
          icon: <NovoNordiskIcon />,
          quote:"Company country entrance strategy in LatAm including brand, product and marketing strategy. Innovation of digital patient and physician engagement tools"
        },
        {
          icon: <EricssonIcon />,
          quote: "Innovation project of new technology for front-and and back-bone"
        },
        {
          icon: <BaseFIcon />,
          quote: "Innovation strategy and conceptual development for iridescent and metallic paint division – incl. special project to BMW"
        }
      ]
    },
    {
      id: 13,
      clients: [
        {
          icon: <BaseFIcon />,
          quote: "Innovation strategy and conceptual development for iridescent and metallic paint division – incl. special project to BMW"
        },
        {
          icon: <CitroFruitIcon />,
          quote:"Product and brand innovation – including product innovation of bi-product material"
        },
        {
          icon: <PoweradeIcon />,
          quote: "Claiming market leader positioning in Europe via Innovation and positioning strategy"
        },
        {
          icon: <AmigoincIcon />,
          quote: "A-to-Z innovation of new umbrella company for cutting edge technology and category solutions; finance, health and credit"
        }
      ]
    },
    {
      id: 14,
      clients: [
        {
          icon: <AmigoincIcon />,
          quote: "A-to-Z innovation of new umbrella company for cutting edge technology and category solutions; finance, health and credit"
        },
        {
          icon: <PuigIcon />,
          quote:"Conceptual and Packaging Innovation for leading luxury brand"
        },
        {
          icon: <TequilaRealIcon />,
          quote: "Repositioning strategy and product packaging innovation and communication"
        }, 
        {
          icon: <BMWIcon />,
          quote: "Conceptual development for BMW 5’series"
        }
      ]
    },
    {
      id: 15,
      clients: [
        {
          icon: <BMWIcon />,
          quote: "Conceptual development for BMW 5’series"
        },
        {
          icon: <SabritasIcon />,
          quote:"Global A-to-Z innovation project for new product category across 4 continents; creation of leading category"
        },
        {
          icon: <RufflesIcon />,
          quote: "Repositioning and new packaging innovation for hero-brand Ruffles"
        },
        {
          icon: <CampbellsIcon />,
          quote: "Product innovation and Re-invention of Campbell’s can – including organoleptic development of new warm/cold category"
        }
      ]
    },
    {
      id: 16,
      clients: [
        {
          icon: <CampbellsIcon />,
          quote: "Product innovation and Re-invention of Campbell’s can – including organoleptic development of new warm/cold category"
        },
        {
          icon: <TequilaPinqIcon />,
          quote:"Innovation of new brand & product category; world’s first naturally pink Tequila"
        },
        {
          icon: <BimboIcon />,
          quote: "Innovation across multiple categories; product, brand and packaging"
        },
        {
          icon: <CocaColaIcon />,
          quote:
            "Innovation of multiple products, brands, strategies, packaging’s and positioning’s across multiple categories, countries and continents"
        }
      ]
    }
  ]
};

export default clientsData;